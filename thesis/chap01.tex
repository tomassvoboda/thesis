...
\begin{proof}
    ...
    \]
    where $u \in L[\re{E}], \quad v_i \in L[\re{F}_i], $ for $ 1 \leq i \leq N$, \; and $w \in L[\re{G}]$. The $N + 1$ states $q_i$ cannot all be distinct, and at least two, say $q_j$ and $q_k$, are equal to the same state $p$. The computation can therefore be written
    \[
