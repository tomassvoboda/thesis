...
The series of following identities expresses the fact that expressions will be \emph{interpreted in a semiring}. That means, that the defined function symbols \X{?} will satisfy the corresponding axioms: $+$ and $\cdot$ are associative, $+$ is commutative, $0$ is an identity element for $+$ and a zero for $\cdot \:$, $1$ is an identity element for $\cdot$, and $\cdot$ is left and right distributive over $+$:
\begin{gather*}
    ...
    \re{E} + 0 \equiv 0 + \re{E} \equiv \re{E} \; , \quad \re{E} \cdot 0 \equiv 0 \cdot \re{E} \equiv 0 \; , \quad \re{E} \cdot 1 \equiv 1 \cdot \re{E} \equiv \re{E} \; ,\\[1ex]
