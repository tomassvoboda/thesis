...
\]
Note that if we consider an alphabet $A$ a language of words of length $1$ over $A$, then the set of words over $A$, denoted by $A^*$, is
\[
...
Let $A$ be an alphabet and let $\{\re{0}, \re{1}, +, \cdot, *\}$ be five function symbols. Constants $\re{0}$ and $\re{1}$ are nullary operations, $+$ and $\cdot$ are binary, and $*$ is unary.

\begin{defn}
    \emph{A rational expression over $A$} is a formula obtained inductively from the letters of $A$ and the functions $\{\re{0}, \re{1}, +, \cdot, *\}$ by the following process:
    \begin{itemize}
...
To be able to simplify the notation by omitting the parentheses in the rational expressions, we need to specify the order of operator precedence for $+, \: \cdot \:$, and $*$. We let $*$ take precedence over $\cdot \:$, which in turn takes precedence over $+ \:$. As for letters, we omit the symbol $\cdot \:$. With this convention we write, for example,
\begin{align*}
...
The series of following identities expresses the fact that expressions will be \emph{interpreted in a semiring}. That means, that the defined function symbols \X{?} will satisfy the corresponding axioms: $+$ and $\cdot$ are associative, $+$ is commutative, $\re{0}$ is an identity element for $+$ and a zero for $\cdot \:$, $\re{1}$ is an identity element for $\cdot$, and $\cdot$ is left and right distributive over $+$:
\begin{gather*}
