\begin{tikzpicture}[my_automaton]
    ...
    \path
        (0) edge [above, bend left = 20] node [near start, above left=-1mm] {$a$} (1)
        (1) edge [above, bend left = 20] node [right] {$b$} (0)
        (1) edge [above, bend left = 20] node [above] {$a$} (2)
        (2) edge [above, bend left = 20] node [below right=-1mm] {$b$} (1)
        (2) edge [above, bend left = 20] node [above right=-1mm] {$a$} (3)
        (3) edge [above, bend left = 20] node [near start, below left=-1mm] {$b$} (2)
