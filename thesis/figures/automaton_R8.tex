\begin{tikzpicture}[->,>=latex]
    ...
    \tikzstyle{cstate}=[state, square, very thick, minimum size=10pt]
    ...
    \path
        (0) edge [above, bend left = 20] node [near start, above left] {$a$} (1)
        (1) edge [above, bend left = 20] node [right] {$b$} (0)
        (1) edge [above, bend left = 20] node [above] {$a$} (2)
        (2) edge [above, bend left = 20] node [below right] {$b$} (1)
