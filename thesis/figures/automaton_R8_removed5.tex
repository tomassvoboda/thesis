\begin{tikzpicture}[my_automaton]
    ...
    \path
        ...
        (0) edge [bend left = 70] node [above] {$X_2$} (4)
        (4) edge [bend right = 50] node [below] {$Y_2$} (0)
        ...
        (6) edge [bend left] node [below left=-1mm] {$a^2$} (0)
        (0) edge [bend left] node [above right=-1mm] {$b^2$} (6)
    ;
\end{tikzpicture}