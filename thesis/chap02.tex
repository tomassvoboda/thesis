...
\begin{proof}
    First we prove the inequalities for the prefix $u$. Let $u = {(w_{k,n})}^i u'$ such that $u'$ is a~proper prefix of $w_{k,n}$. Since
    \begin{multline*}
    ...
    \[
        w_{k,n} = a^{2^k} {(w_{k-1,n})}^n b^{2^k} {(w_{k-1,n})}^n
    \]
    begins with $2^k$ $a$'s, therefore if $u$ is a~prefix of $a^{2^k} {(w_{k-1,n})}^n$, $0 \leq |u|_a - |u|_b \leq 2^k+2^k-1 = 2^{k+1}-1$. That proves the inequalities fot the prefix $u$.
